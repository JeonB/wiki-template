...
function markdownUnquotedDate(title, body) {
  return `---\ntitle: ${title}\ncategory: Regression\nupdatedAt: 2026-01-01T00:00:00.000Z\n---\n\n# ${title}\n\n${body}\n`;
}
...
async function main() {
  ...
  try {
    await Promise.all([
      ...
      writeFile(
        join(contentDir, 'yaml-date.md'),
        markdownUnquotedDate('Yaml Date', 'Unquoted YAML dates must stay editable.'),
      ),
    ]);
    ...
    const yamlDateEdit = await fetchText(`${baseUrl}/yaml-date/edit`);
    assert(
      yamlDateEdit.response.status === 200,
      `Expected /yaml-date/edit 200, got ${yamlDateEdit.response.status}`,
    );
    assert(
      yamlDateEdit.text.includes('name="expectedUpdatedAt" value="2026-01-01T00:00:00.000Z"'),
      'Expected unquoted YAML updatedAt to serialize as a stable ISO string for edit revisions',
    );
